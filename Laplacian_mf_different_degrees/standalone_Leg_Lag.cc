// Solves the Poisson equation -\Delta u = f on the rectangle [0,Lx]x[0,\infty] with Neumann bcs on
// the lower boundary and Dirichlet bcs elsewhere using SIPG and a matrix free implementation. We use Legendre
// polynomials in the x-direction and scaled Laguerre functions in the z-direction
...
#include "FE_Laguerre.h"
#include "FE_LaguerreFunction.h"
...
const unsigned int fe_degree_x = 3, fe_degree_z = 10;


/*----------------------------------------------------- Solution -------------------------------------------------------------*/
// We impose the manufactured solution u(x,z) = exp(-z)(z^2-2z)*cos(2*pi*x)
template <int dim>
...
public:
  ...
  {
    return std::exp(-p[1]) * (p[1] * p[1] - 2*p[1]) * std::cos(2 * numbers::PI * p[0]);
  }
  ...
  {
    Tensor<1, dim> grad;
    grad[0] = -2*numbers::PI*std::sin(2*numbers::PI*p[0])*std::exp(-p[1])*(p[1]*p[1]-2*p[1]);
    grad[1] = std::cos(2*numbers::PI*p[0])*std::exp(-p[1])*(-p[1]*p[1]+4*p[1]-2);
    return grad;
  }
};
...
public:
  virtual double value(const Point<dim> &p,
                       const unsigned int = 0) const override final
  {
    return -exp(-p[1])*cos(2*numbers::PI*p[0])*(- std::pow(2*numbers::PI*p[1],2) + 
        8*std::pow(numbers::PI,2)*p[1] + std::pow(p[1],2) - 6*p[1] + 6);
  }
};
      ...
      const std::pair<unsigned int, unsigned int>      &face_range) const
{
  ...
  for (unsigned int face = face_range.first; face < face_range.second; ++face)
  {
    ...
    if (!(!phi_inner.is_vertical && phi_inner.is_right_or_top))
    {
    // read dof values
    phi_inner.read_dof_values(src);
    phi_inner.evaluate(EvaluationFlags::values | EvaluationFlags::gradients);
...
{
  public:
    LaplaceProblem(double _mu, double _Lz);
  ...
  private:
    ...
    FE_DGQLegendre<1>         fe_x;
    FE_LaguerreFunction<1>    fe_z;
    QGauss<1>                 quad_x;
    QGaussLaguerreRadau<1>    quad_z;
...
template <int dim>
LaplaceProblem<dim>::LaplaceProblem(double _mu, double _Lz)
  // in theory, the tensor product finite element space should be defined via FE_Enriched because we are using
  // Laguerre functions in the z-direction. However, we are only going to use fe to initialize the dof_handler,
  // and this does not depend on the enrichment. For the rest, the fact that we are using a matrix free implementation
  // ensures that we can restrict the enrichment inside FE_LaguerreFunction, and define fe as a simple
  // FE_DGQ_Aniso, which makes the code more general
  : fe(fe_degree_x, fe_degree_z, 
            AnisotropicPolynomials<dim>({Polynomials::Legendre::generate_complete_basis(fe_degree_x),
            Laguerre::generate_complete_basis_scaled(fe_degree_z, _mu)}))
  , dof_handler(triangulation)
  , fe_x(fe_degree_x), fe_z(fe_degree_z, _mu*_Lz, _Lz)
  , quad_x(fe_degree_x+1), quad_z(fe_degree_z, _mu*_Lz)
  , mu(_mu), Lz(_Lz)
{}
...
void LaplaceProblem<dim>::setup_system()
{
  ...
  const std::vector<const FiniteElement<1> *> fe_vector{&fe_x, &fe_z};
  const std::vector<Quadrature<1>>            q_vector{quad_x, quad_z};
...
void LaplaceProblem<dim>::assemble_rhs()
{
      ...
      for (const unsigned int q : phi.quadrature_point_indices())
        {
          ...
          //std::cout << "submitted value: " << rhs_val << "      " << phi.begin_values()[q] << std::endl;
        }
           ...
           ++face)
  {
    ...
    if (!(!phi_face.is_vertical && phi_face.is_right_or_top))
    {
    for (unsigned int q = 0; q < face_q_points; q++)
      {
...
{
  // Remember to set a large fe_degree_z
  const double Lx = 1; 
  const double mu = 1;
  const unsigned int Nz = 1;

  const double Lz = Laguerre(fe_degree_z+1, mu).get_zeros_of_derivative()[fe_degree_z-1];
  ConvergenceTable convergence_table; 

  for (unsigned int cycle = 1; cycle < 10; cycle++) 
  {
    LaplaceProblem<dim> lp(mu, Lz);
  ...
  convergence_table.evaluate_convergence_rates("L2err", "Nx", ConvergenceTable::reduction_rate_log2, 1);
...
{
  const double Lx = 1;
  const unsigned int Nx = 10, Nz = 1;
  const double mu = 1;

  // In theory, the computational domain is unbounded. However, in practice we set the upper limit as the last
  // Laguerre quadrature point
  const double Lz = Laguerre(fe_degree_z+1, mu).get_zeros_of_derivative()[fe_degree_z-1];

  LaplaceProblem<dim> lp1(mu, Lz);
  lp1.run(Lx, Lz, Nx, Nz);
