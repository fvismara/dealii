...
/*
Solves the Poisson equation -\Delta u = f with Dirichlet boundary conditions using the SIPG method. Builds on step 74
*/
 
 
namespace XDGLaplacian
{
  ...
  void RhsFunction<dim>::value_list(const std::vector<Point<dim>> &points, std::vector<double> &values, const unsigned int component) const
  {
    ...
    for (unsigned int i = 0; i < values.size(); ++i)
      values[i] = -std::exp(-points[i][1])*std::sin(2*numbers::PI*points[i][0]/Lx)*((1-std::pow(2*numbers::PI/Lx,2))*points[i][1]-2);
  }

  // compute penalty coefficient for SIPG formulation
  double get_penalty_factor(const unsigned int fe_degree, const double cell_extent_left, const double cell_extent_right)
  {
  ...
  {
  public:
    LaplacianProblem(unsigned int _px, unsigned int _pz, unsigned int _M, unsigned int _Nx, unsigned int _Nz,
  ...
  template <int dim>
  LaplacianProblem<dim>::LaplacianProblem(unsigned int _px, unsigned int _pz, unsigned int _M, unsigned int _Nx, 
  ...
  template <int dim>
  void LaplacianProblem<dim>::setup_system()
  {
  ...
  template <int dim>
  void LaplacianProblem<dim>::assemble_system()
  {
          ...
          for (unsigned int i = 0; i < n_dofs; ++i)
            for (unsigned int j = 0; j < n_dofs; ++j)
                copy_data.cell_matrix(i, j) +=
                  ...
                  * fe_cell.shape_grad(i, qpoint)  // \phi_i
                  * fe_cell.shape_grad(j, qpoint)  // \phi_j
    ...
    const auto boundary_worker = [&](const Iterator &cell, const unsigned int &face_no, ScratchData<dim> &scratch_data, CopyData &copy_data) {
      ...
      const double penalty = get_penalty_factor(10, extent1, extent1);
              ...
              for (unsigned int j = 0; j < n_dofs; ++j)
                copy_data.cell_matrix(i, j) +=
                  (-diffusion_coefficient *             // - nu
                    fe_face.shape_value(i, qpoint) *    // v_h
                    (fe_face.shape_grad(j, qpoint) *    // (grad u_h .
                     normals[qpoint])                   //  n)
  
                   - diffusion_coefficient *            // - nu
                    (fe_face.shape_grad(i, qpoint) *    // (grad v_h .
                     normals[qpoint]) *                 //  n)
                    fe_face.shape_value(j, qpoint)      // u_h
              ...
              copy_data.cell_rhs(i) += 
                (-diffusion_coefficient *                       // - nu
                  (fe_face.shape_grad(i, qpoint) *              // (grad v_h .
                   ...
                   g[qpoint]                                    // g
                 + diffusion_coefficient * penalty *            // + nu sigma
                   fe_face.shape_value(i, qpoint) * g[qpoint])  // v_h g
    ...
    const auto face_worker = [&](const Iterator     &cell,
      ...
      const double penalty = get_penalty_factor(10, extent1, extent2);
          ...
          for (unsigned int i = 0; i < n_dofs; ++i)
            for (unsigned int j = 0; j < n_dofs; ++j)
              copy_data_face.cell_matrix(i, j) +=
                (-diffusion_coefficient *                        // - nu
                  fe_iv.jump_in_shape_values(i, qpoint) *        // [v_h]
                  (fe_iv.average_of_shape_gradients(j, qpoint) * // ({grad u_h} .
                   normals[qpoint])                              //  n)
  
                 - diffusion_coefficient *                       // - nu
                  (fe_iv.average_of_shape_gradients(i, qpoint) * // (grad v_h .
                   normals[qpoint]) *                            //  n)
                  fe_iv.jump_in_shape_values(j, qpoint)          // [u_h]
  ...
  template <int dim>
  void LaplacianProblem<dim>::solve()
  {
  ...
  template <int dim>
  void LaplacianProblem<dim>::output_results() const
  {
  ...
  template <int dim>
  void LaplacianProblem<dim>::compute_error()
  {
...
void convergence_analysis (unsigned int px, unsigned int pz, unsigned int M, unsigned int Nx, unsigned int Nz,
  ...
  for (unsigned int cycle = 0; cycle < n_refinements; cycle++) {
    XDGLaplacian::LaplacianProblem<2> XDG_laplacian(px, pz, _M, _Nx, _Nz, mu, Lx, Lz);
    XDG_laplacian.run();

    convergence_table.add_value("Nx", _Nx);
    convergence_table.add_value("Nz", _Nz);
    convergence_table.add_value("M", _M);
    convergence_table.add_value("L2err", XDG_laplacian.get_L2_error());
...
int main()
{
  ...
  bool run_convergence_test = false; // set to true for convergence test, false to solve the problem once
  ...
  if (run_convergence_test)
    ...
    XDGLaplacian::LaplacianProblem<2> XDG_laplacian(px, pz, M, Nx, Nz, mu, Lx, Lz);
    XDG_laplacian.run();
    std::cout << XDG_laplacian.get_L2_error() << std::endl;
